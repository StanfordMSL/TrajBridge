// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from px4_msgs:msg/TransponderReport.idl
// generated code does not contain a copyright notice
#include "px4_msgs/msg/detail/transponder_report__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
px4_msgs__msg__TransponderReport__init(px4_msgs__msg__TransponderReport * msg)
{
  if (!msg) {
    return false;
  }
  // timestamp
  // icao_address
  // lat
  // lon
  // altitude_type
  // altitude
  // heading
  // hor_velocity
  // ver_velocity
  // callsign
  // emitter_type
  // tslc
  // flags
  // squawk
  // uas_id
  return true;
}

void
px4_msgs__msg__TransponderReport__fini(px4_msgs__msg__TransponderReport * msg)
{
  if (!msg) {
    return;
  }
  // timestamp
  // icao_address
  // lat
  // lon
  // altitude_type
  // altitude
  // heading
  // hor_velocity
  // ver_velocity
  // callsign
  // emitter_type
  // tslc
  // flags
  // squawk
  // uas_id
}

bool
px4_msgs__msg__TransponderReport__are_equal(const px4_msgs__msg__TransponderReport * lhs, const px4_msgs__msg__TransponderReport * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // timestamp
  if (lhs->timestamp != rhs->timestamp) {
    return false;
  }
  // icao_address
  if (lhs->icao_address != rhs->icao_address) {
    return false;
  }
  // lat
  if (lhs->lat != rhs->lat) {
    return false;
  }
  // lon
  if (lhs->lon != rhs->lon) {
    return false;
  }
  // altitude_type
  if (lhs->altitude_type != rhs->altitude_type) {
    return false;
  }
  // altitude
  if (lhs->altitude != rhs->altitude) {
    return false;
  }
  // heading
  if (lhs->heading != rhs->heading) {
    return false;
  }
  // hor_velocity
  if (lhs->hor_velocity != rhs->hor_velocity) {
    return false;
  }
  // ver_velocity
  if (lhs->ver_velocity != rhs->ver_velocity) {
    return false;
  }
  // callsign
  for (size_t i = 0; i < 9; ++i) {
    if (lhs->callsign[i] != rhs->callsign[i]) {
      return false;
    }
  }
  // emitter_type
  if (lhs->emitter_type != rhs->emitter_type) {
    return false;
  }
  // tslc
  if (lhs->tslc != rhs->tslc) {
    return false;
  }
  // flags
  if (lhs->flags != rhs->flags) {
    return false;
  }
  // squawk
  if (lhs->squawk != rhs->squawk) {
    return false;
  }
  // uas_id
  for (size_t i = 0; i < 18; ++i) {
    if (lhs->uas_id[i] != rhs->uas_id[i]) {
      return false;
    }
  }
  return true;
}

bool
px4_msgs__msg__TransponderReport__copy(
  const px4_msgs__msg__TransponderReport * input,
  px4_msgs__msg__TransponderReport * output)
{
  if (!input || !output) {
    return false;
  }
  // timestamp
  output->timestamp = input->timestamp;
  // icao_address
  output->icao_address = input->icao_address;
  // lat
  output->lat = input->lat;
  // lon
  output->lon = input->lon;
  // altitude_type
  output->altitude_type = input->altitude_type;
  // altitude
  output->altitude = input->altitude;
  // heading
  output->heading = input->heading;
  // hor_velocity
  output->hor_velocity = input->hor_velocity;
  // ver_velocity
  output->ver_velocity = input->ver_velocity;
  // callsign
  for (size_t i = 0; i < 9; ++i) {
    output->callsign[i] = input->callsign[i];
  }
  // emitter_type
  output->emitter_type = input->emitter_type;
  // tslc
  output->tslc = input->tslc;
  // flags
  output->flags = input->flags;
  // squawk
  output->squawk = input->squawk;
  // uas_id
  for (size_t i = 0; i < 18; ++i) {
    output->uas_id[i] = input->uas_id[i];
  }
  return true;
}

px4_msgs__msg__TransponderReport *
px4_msgs__msg__TransponderReport__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  px4_msgs__msg__TransponderReport * msg = (px4_msgs__msg__TransponderReport *)allocator.allocate(sizeof(px4_msgs__msg__TransponderReport), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(px4_msgs__msg__TransponderReport));
  bool success = px4_msgs__msg__TransponderReport__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
px4_msgs__msg__TransponderReport__destroy(px4_msgs__msg__TransponderReport * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    px4_msgs__msg__TransponderReport__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
px4_msgs__msg__TransponderReport__Sequence__init(px4_msgs__msg__TransponderReport__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  px4_msgs__msg__TransponderReport * data = NULL;

  if (size) {
    data = (px4_msgs__msg__TransponderReport *)allocator.zero_allocate(size, sizeof(px4_msgs__msg__TransponderReport), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = px4_msgs__msg__TransponderReport__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        px4_msgs__msg__TransponderReport__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
px4_msgs__msg__TransponderReport__Sequence__fini(px4_msgs__msg__TransponderReport__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      px4_msgs__msg__TransponderReport__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

px4_msgs__msg__TransponderReport__Sequence *
px4_msgs__msg__TransponderReport__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  px4_msgs__msg__TransponderReport__Sequence * array = (px4_msgs__msg__TransponderReport__Sequence *)allocator.allocate(sizeof(px4_msgs__msg__TransponderReport__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = px4_msgs__msg__TransponderReport__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
px4_msgs__msg__TransponderReport__Sequence__destroy(px4_msgs__msg__TransponderReport__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    px4_msgs__msg__TransponderReport__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
px4_msgs__msg__TransponderReport__Sequence__are_equal(const px4_msgs__msg__TransponderReport__Sequence * lhs, const px4_msgs__msg__TransponderReport__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!px4_msgs__msg__TransponderReport__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
px4_msgs__msg__TransponderReport__Sequence__copy(
  const px4_msgs__msg__TransponderReport__Sequence * input,
  px4_msgs__msg__TransponderReport__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(px4_msgs__msg__TransponderReport);
    px4_msgs__msg__TransponderReport * data =
      (px4_msgs__msg__TransponderReport *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__TransponderReport__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__TransponderReport__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!px4_msgs__msg__TransponderReport__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
