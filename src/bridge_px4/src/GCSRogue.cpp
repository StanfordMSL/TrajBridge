...
void GCSRogue::load_trajectory(const string& input)
{
    ifstream data(input);
    if (data.is_open()) {
        int rows = 0;
...
void GCSRogue::rc_takeoff_sequence(const int &n_dr)
{
...
void GCSRogue::rc_rtl_sequence(const int &n_dr)
{
    mavros_msgs::SetMode set_mode;
    string drones = "";
    string output_msg = "";
...
void GCSRogue::pos_sp_init(const int &n_dr) {
    for (int i=0; i<n_dr; i++) {
...
{
    ros::init(argc, argv, "gcsrogue_node");

    GCSRogue gcsrogue;

    ros::Rate rate(100);
    while(ros::ok()){
        gcsrogue.update_setpoint();
