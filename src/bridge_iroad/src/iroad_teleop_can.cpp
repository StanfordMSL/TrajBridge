...
Teleop_Can::Teleop_Can():
  ...
  sp_conv(1.0/3.6), //Factor to convert vehicle speed signal to meters per second
  //ln_conv(M_PI/180.0), //Factor to convert vehicle lean signal to radians
  whlbase(1.695) //vehicle wheelbase, in meters
{
  n.param("whlbase",whlbase,whlbase);

  n.param("st_conv",st_conv,st_conv);
  n.param("sp_conv",sp_conv,sp_conv);
  ...
  len_pub_ = n.advertise<int32_t>("can_lean_ang", 10);

  double hz = 10.0; //Refresh rate of CAN feedback shown by display
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    //NOTE: This twist describes the current maneuver taken by the vehicle but discounts the effects of vehicle lean, only accounting for vehicle speed (m/s) and turn rate (rad/s)
    teleop_can.can_twist.linear.y = teleop_can.can_feedback.can_vspd * teleop_can.sp_conv; //y-axis of body frame aims in the forward direction of the vehicle
    teleop_can.can_twist.angular.z = -teleop_can.can_feedback.can_vspd*teleop_can.sp_conv*tan(teleop_can.can_feedback.can_steer*teleop_can.st_conv)/teleop_can.whlbase; //z-axis of body frame points straight down (matching IMU configuration), effects of tilt are discounted for now
    ...
    teleop_can.len_pub_.publish(teleop_can.can_feedback.can_lean);
